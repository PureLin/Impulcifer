...
class PositionTracker:
    def __init__(self, positionVar: tkinter.StringVar):
        self.positionVar = positionVar
        self.positionVar.set('_,_,_')
        # open udp port for receive position from client
        self.udp = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.udp.bind(('', 8888))
        self.udp.settimeout(0.1)
        # create a thread, for receive position from client
    ...
    def receive(self):
        ...
        while not self.stop:
            try:
                self.data = self.udp.recvfrom(1024)
                self.positionVar.set(self.data[0].decode('utf-8').replace('.', ','))
...
def pack(widget, samerow=False, align='left'):
    ...
    if align == 'left':
        ...
        widget.place(x=pos[0], y=pos[1], anchor=E)
...
process = subprocess.Popen(["F:/Project/VRGUI/VirtualSurroundGUI/bin/Debug/net6.0-windows/VirtualSurroundGUI.exe"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def exit_program():
    ...
    time.sleep(0.1)
    position_tracker.stop = True
    root.destroy()


exit_button = Button(canvas1, text='EXIT', command=exit_program)
pack(exit_button, align='right')
...
position_tracker = PositionTracker(position)
position_entry = Entry(canvas1, textvariable=position, width=70)
pack(position_entry)
...
def recordaction():
    ...
    try:
        ...
        if inputVar2.get() != 0:
            input_map += ("," + str(5))
        postion_str = position.get().replace(" ", "")
        recorder.play_and_record(play=play_l.get(),
                                 record=record_entry.get() + "//FL//" + postion_str + ".wav",
        ...
        recorder.play_and_record(play=play_r.get(),
                                 record=record_entry.get() + "//FR//" + postion_str + ".wav",
                                 input_device=input_device.get(),
                                 output_device=output_device.get(),
                                 host_api=host_api.get(),
                                 output_mapping=str(speakerVar.get()) + "," + str(speakerVar.get() + 1),
...
def start_impulcifer():
    ...
    if not os.path.exists(temp_dir + "/test.wav"):
        ...
        return
    # copy headphone and test file to FL and FR dir
    shutil.copy(temp_dir + "/test.wav", temp_dir + "/FL/test.wav")
    shutil.copy(temp_dir + "/test.wav", temp_dir + "/FR/test.wav")
    if os.path.exists(temp_dir + "/headphones.wav"):
        shutil.copy(temp_dir + "/headphones.wav", temp_dir + "/FL/headphones.wav")
        shutil.copy(temp_dir + "/headphones.wav", temp_dir + "/FR/headphones.wav")
    # run impulcifer in FL and FR dir
    impulcifer.main(dir_path=temp_dir + "/FL/", channel_type="brir_random",
                    do_headphone_compensation=do_headphone_compensation,
                    use_reference_channel=inputVar2.get() != 0, normalize_gain=False)
    impulcifer.main(dir_path=temp_dir + "/FR/", channel_type="brir_random",
                    do_headphone_compensation=do_headphone_compensation,
                    use_reference_channel=inputVar2.get() != 0, normalize_gain=False)
    # delete result dir first
    if os.path.exists(temp_dir + "/result"):
        shutil.rmtree(temp_dir + "/result")
    # move result dir to result dir
    shutil.move(temp_dir + "/FL/result/48/", temp_dir + "/result/FL")
    shutil.move(temp_dir + "/FR/result/48/", temp_dir + "/result/FR")
    errorLabel.config(text='')
...
keyboard.add_hotkey('f1', lambda: main_button.invoke())
keyboard.add_hotkey('f2', lambda: headphones.invoke())
