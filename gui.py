...
record = StringVar(value=os.path.join('data', 'my_hrir', 'FL.wav'))
...
ToolTip(channels_checkbutton,
        'For room correction: some measurement microphones like MiniDSP UMIK-1 are seen as stereo microphones by Windows and will for that reason record a stereo file. recorder can force the capture to be one channel')
channels = IntVar(value=1)
channels_entry = Entry(canvas1, textvariable=channels, width=5, validate='key',
...
# record button
def recordaction():
    recorder.play_and_record(play=play_entry.get(), record=record_entry.get(), input_device=input_device.get(),
                             output_device=output_device.get(), host_api=host_api.get(),
                             channels=(channels.get() if channels_check.get() else 2), append=append.get())
...
def refresh2(changedpath=False):
    ...
    if do_room_correction.get():
        do_room_correction_msg_label.place(x=label_pos[do_room_correction_msg_label][0],
                                           y=label_pos[do_room_correction_msg_label][1], anchor=W)
    else:
        do_room_correction_msg_label.place_forget()
    if do_headphone_compensation.get():
        do_headphone_compensation_msg_label.place(x=label_pos[do_headphone_compensation_msg_label][0],
                                                  y=label_pos[do_headphone_compensation_msg_label][1], anchor=W)
    else:
        do_headphone_compensation_msg_label.place_forget()
    if do_equalization.get():
        do_equalization_msg_label.place(x=label_pos[do_equalization_msg_label][0],
                                        y=label_pos[do_equalization_msg_label][1], anchor=W)
    else:
    ...
    if show_adv.get():
        ...
        if channel_balance.get() == 'number':
            channel_balance_db_entry.place(x=adv_options_pos[channel_balance_db_entry][0],
                                           y=adv_options_pos[channel_balance_db_entry][1], anchor=W)
            channel_balance_db_label.place(x=adv_options_pos[channel_balance_db_label][0],
                                           y=adv_options_pos[channel_balance_db_label][1], anchor=W)
        else:
        ...
        if decay_per_channel.get():
            for i in range(7):
                decay_labels[i].place(x=adv_options_pos[decay_labels[i]][0], y=adv_options_pos[decay_labels[i]][1],
                                      anchor=W)
                decay_entries[i].place(x=adv_options_pos[decay_entries[i]][0], y=adv_options_pos[decay_entries[i]][1],
                                       anchor=W)
        else:
            for i in range(7):
...
pack(Button(canvas2, text='...',
            command=lambda: openfile(test_signal, (('Audio files', '*.wav *.pkl'), ('All files', '*.*')))),
     samerow=True)

# room correction
label_pos = {}
do_room_correction = BooleanVar()
do_room_correction_checkbutton = Checkbutton(canvas2, text="Room correction ", variable=do_room_correction,
                                             command=lambda: refresh2(
                                                 changedpath=True if do_room_correction.get() else False))
ToolTip(do_room_correction_checkbutton,
...
specific_limit_entry = Entry(canvas2, textvariable=specific_limit, width=5, validate='key',
                             vcmd=(root.register(validate_int), '%P'))
...
generic_limit_entry = Entry(canvas2, textvariable=generic_limit, width=5, validate='key',
                            vcmd=(root.register(validate_int), '%P'))
...
ToolTip(fr_combination_method_label,
...
ToolTip(room_mic_calibration_label,
...
pack(Button(canvas2, text='...',
            command=lambda: openfile(room_mic_calibration, (('Text files', '*.csv *.txt'), ('All files', '*.*')))),
     samerow=True)
...
pack(Button(canvas2, text='...',
            command=lambda: openfile(room_target, (('Text files', '*.csv *.txt'), ('All files', '*.*')))), samerow=True)

# headphone compensation
do_headphone_compensation = BooleanVar()
do_headphone_compensation_checkbutton = Checkbutton(canvas2, text="Headphone compensation ",
                                                    variable=do_headphone_compensation, command=lambda: refresh2(
        changedpath=True if do_headphone_compensation.get() else False))
ToolTip(do_headphone_compensation_checkbutton,
...
do_equalization_checkbutton = Checkbutton(canvas2, text="Custom EQ", variable=do_equalization, command=lambda: refresh2(
    changedpath=True if do_equalization.get() else False))
ToolTip(do_equalization_checkbutton,
...
target_level_entry = Entry(canvas2, textvariable=target_level, width=7, validate='key',
                           vcmd=(root.register(validate_double), '%P'))
ToolTip(target_level_label,
...
bass_boost_gain_entry = Entry(canvas2, textvariable=bass_boost_gain, width=7, validate='key',
                              vcmd=(root.register(validate_double), '%P'))
...
bass_boost_fc_entry = Entry(canvas2, textvariable=bass_boost_fc, width=7, validate='key',
                            vcmd=(root.register(validate_int), '%P'))
...
bass_boost_q_entry = Entry(canvas2, textvariable=bass_boost_q, width=7, validate='key',
                           vcmd=(root.register(validate_double), '%P'))
...
channel_balance_db_entry = Entry(canvas2, textvariable=channel_balance_db, width=5, validate='key',
                                 vcmd=(root.register(validate_double), '%P'))
...
# impulcify button
def impulcify():
    args = {'dir_path': dir_path.get(), 'test_signal': test_signal.get(), 'plot': plot.get(),
            'do_room_correction': do_room_correction.get(),
            'do_headphone_compensation': do_headphone_compensation.get(), 'do_equalization': do_equalization.get()}
    if do_room_correction.get():
    ...
    if show_adv.get():
        ...
        args['channel_balance'] = channel_balance_db.get() if channel_balance.get() == 'number' else (
        ...
        args['tilt'] = tilt.get()
        if decay_per_channel.get():
            args['decay'] = {decay_labels[i].cget('text'): float(decay_entries[i].get()) / 1000 for i in range(7) if
                             decay_entries[i].get()}
        elif decay.get():
